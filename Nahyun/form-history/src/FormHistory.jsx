// src/FormHistoryLab.jsx
import { useCallback, useEffect, useMemo, useRef, useState } from 'react';
import { AsyncQueue, Stack, clone } from './utils/stackQueue';
...
export default function FormHistoryLab() {
  ...
  const applyChange = useCallback(
    (patch) => {
      ...
      setForm((prev) => {
        ...
        // Undo/Redo 작업이 아닌 경우에만 back push
  ...
  }, [redo, saveNow, undo]); // 힌트: form이 바뀔 때 최신 undo/redo 대상 보장
  ...
  const serverDoc = useMemo(() => {
    ...
    // logs가 변할 때마다 localStorage에서 최신 저장본을 읽어옴
    const s = localStorage.getItem('serverDoc');
    return s ? JSON.parse(s) : null;
  }, [logs]); // 저장 이벤트가 있을 때 갱신
      ...
      <div className='grid grid-cols-1 gap-4 md:grid-cols-2'>
        ...
        <div className='rounded-lg border border-gray-200 bg-white p-4'>
          ...
          <p className='mt-2 text-xs text-gray-500'>
            * Autosave가 켜져 있으면 변경 때마다 저장 작업이{' '}
            <strong>큐에 직렬로</strong> 들어가서 순서대로 처리됩니다. 네트워크
            에러가 뜨면 큐에서 <strong>error</strong> 로그로 확인할 수 있습니다.
          </p>
        </div>
      </div>
    </section>
