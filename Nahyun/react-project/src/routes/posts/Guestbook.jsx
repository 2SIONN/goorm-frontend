...
export default function Guestbook() {
  ...
  const createPost = useMutation({
    ...
    onMutate: async (payload) => {
      ...
      qc.setQueryData(POSTS_KEY, (prev) => {
        if (!prev) {
          return {
            items: [tempPost],
    ...
    onSuccess: (res, _payload, ctx) => {
      ...
      qc.setQueryData(POSTS_KEY, (prev) => {
        ...
        return {
          ...prev,
          items: (prev.items ?? []).map((p) => (p._id === ctx?.tempId ? res.post : p)),
        }
      })
    },
    onSettled: () => {
  ...
  const deletePost = useMutation({
    ...
    onMutate: async (id) => {
      ...
      // 현재 캐시에 저장된 posts 데이터를 백업 getQueryData
      const prevData = qc.getQueryData(POSTS_KEY)
      // 캐시 데이터 수정 (낙관적 업데이트) setQueryData
      qc.setQueryData(POSTS_KEY, (prev) => {
        if (!prev) {
          return prev
        }
        return {
          ...prev,
          items: (prev.items ?? []).filter((p) => p._id !== id),
    ...
    onError: (_err, _id, ctx) => {
      ...
      // 실패했다면 이전 상태(prev)로 복구 (rollback) setQueryData
      if (ctx?.prevData) {
        qc.setQueryData({ queryKey: ['posts'] })
      }
    },
    onSettled: () => {
      // 서버와 캐시 데이터 동기화 (혹시 누락된 글이 있다면 다시 가져오기) invalidateQueries
