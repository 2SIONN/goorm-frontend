...
export default function SearchAutoComplete() {
  ...
  const norm = (s) => s.normalize().toLowerCase().split(' ').join('');

  const highlight = (text, query) => {
    const i = text.toLowerCase().indexOf(query.toLowerCase());
    return i === -1 ? text : <>{text.slice(0,i)}<strong>{text.slice(i,i+query.length)}</strong>{text.slice(i+query.length)}</>;
  };
  ...
  const results = useMemo(() => {
    ...
    return BOOKS.filter((book) => {
      return norm(book).includes(norm(q));
    }).slice(0, 8);
  }, [q]);
  ...
  return (
    <div className='mx-auto max-w-[560px] p-6'>
      ...
      <ul className='mt-3 list-none divide-y divide-gray-100 rounded-lg border border-gray-100 bg-white'>
        {results.map((title) => (
          <li key={title} className='px-3 py-2 text-gray-800 hover:bg-gray-50'>
            {highlight(title, q)}
          </li>
        ))}
        {q && results.length === 0 && (
      ...
      <p className='mt-2 text-sm text-gray-600'>
        현재 구현: 배열 전체를 매번 순회(선형 탐색 O(n)). 데이터가 아주 크면
        버벅일 수 있음.
      </p>
    </div>
  );
